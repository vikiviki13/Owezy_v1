...
export function SettingsSection({ title, children }: { title: string; children: ReactNode }) {
  return (
    <section className="mb-7">
      <h2 className="text-xs uppercase tracking-wide font-bold text-[var(--color-text-muted)] px-1 mb-2">{title}</h2>
...
export function SettingsRow({ icon: Icon, title, description, value, to, onClick, danger = false, trailing = true, disabled = false }: {
  icon: LucideIcon; title: string; description?: string; value?: string; to?: string; onClick?: () => void; danger?: boolean; trailing?: boolean; disabled?: boolean;
}) {
  const content = (
    <>
      <span className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${danger ? 'bg-destructive/10 text-destructive' : 'bg-[var(--color-surface-secondary)] text-[var(--color-text-secondary)]'}`}><Icon size={18} /></span>
      <span className="min-w-0 flex-1 text-left">
        <span className={`block text-sm font-medium ${danger ? 'text-destructive' : ''}`}>{title}</span>
  ...
  const cls = `w-full min-h-14 px-3.5 py-3 flex items-center gap-3 active:bg-[var(--color-surface-secondary)] transition-colors ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`;
  return to ? <Link to={to} className={cls}>{content}</Link> : <button type="button" onClick={onClick} disabled={disabled} className={cls}>{content}</button>;
}
...
export function ChoiceRow({ selected, title, description, onClick }: { selected: boolean; title: string; description?: string; onClick: () => void }) {
  return (
    <button onClick={onClick} className="w-full min-h-[64px] px-4 py-3 flex items-center gap-3 text-left active:bg-[var(--color-surface-secondary)]">
      ...
      <span className="flex-1"><span className="block font-semibold text-sm">{title}</span>{description && <span className="block text-xs text-[var(--color-text-muted)] mt-1">{description}</span>}</span>
    </button>
  );
}
