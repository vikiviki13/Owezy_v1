...
export function SecurityDevices() {
  ...
  return <SettingsPage title="Device Authentication" description="Registered credentials can verify you with device security. Private keys and biometric information never leave the authenticator.">
    <button type="button" onClick={add} disabled={adding} className="w-full min-h-12 rounded-xl bg-[var(--color-primary)] text-white font-semibold flex items-center justify-center gap-2 mb-7 disabled:opacity-50">{adding ? <LoaderCircle className="animate-spin" size={18} /> : <Plus size={18} />}Add This Device</button>
...
export function AccountRecoveryPage() {
  const { status, unlockWithDevice, refresh } = useSecurity();
  const toast = useToast();
  ...
  const verifyAccount = useCallback(async (event: FormEvent) => {
    ...
    try { const { data: userData } = await supabase.auth.getUser(); const email = userData.user?.email; if (!email) throw new Error('Your account email is unavailable.'); const { error: signInError } = await supabase.auth.signInWithPassword({ email, password }); if (signInError) throw signInError; setVerified(true); setPassword(''); }
  ...
  const reset = useCallback(async (pin: string) => { setBusy(true); setError(''); try { const { data } = await supabase.auth.getUser(); if (!data.user) throw new Error('Please sign in again.'); await recoverPin(data.user.id, pin, status?.autoLockDuration || '5m'); await refresh(); toast('App PIN reset successfully'); window.location.hash = '#/'; } catch (caught) { setError(caught instanceof Error ? caught.message : 'PIN recovery failed.'); } finally { setBusy(false); } }, [refresh, status?.autoLockDuration, toast]);
